...
//    centralWidget()->setMouseTracking(true);
    ...
    QAction *actionEVE=new QAction("Computer VS Computer",this);
    connect(actionEVE,SIGNAL(triggered()),this,SLOT(initEVEGame()));
...
//    painter.setPen(pen);
        ...
        game->gameMapVec[clickPosRow][clickPosCol] == 0)
    {
        ...
        painter.drawRect(kBoardMargin + kBlockSize * clickPosCol - kMarkSize / 2-20, kBoardMargin + kBlockSize * clickPosRow - kMarkSize / 2-20, kMarkSize, kMarkSize);
    }
...
//                qDebug()<<i<<j<<endl;
                brush.setColor(Qt::white);
                painter.setBrush(brush);
                painter.drawEllipse(kBoardMargin + kBlockSize * j - kRadius-20, kBoardMargin + kBlockSize * i - kRadius-20, kRadius * 2, kRadius * 2);
            }
    ...
    // 判断输赢
    if (clickPosRow > 0 && clickPosRow < kBoardSizeNum+1 &&
        clickPosCol > 0 && clickPosCol < kBoardSizeNum+1 &&
...
{   
            ...
            y < size().height()- kBoardMargin)
    {
        ...
        int leftTopPosX = kBoardMargin + kBlockSize * col-20;
        int leftTopPosY = kBoardMargin + kBlockSize * row-20;
...
//        QTimer::singleShot(kAIDelay, this, SLOT(chessOneByAI()));
    ...
    }else if(game_type == BOTVS && game->playerFlag)
        chessOneByAI();
    if(game_type==BOTVS&&!game->playerFlag)
        QTimer::singleShot(kAIDelay, this, SLOT(chessOneByAI()));
}
